...
const navigationItems = [
  ...
  {
    key: "warmup-test" as const,
    label: "Teste Warmup API",
    icon: Zap,
    badge: "TEST",
    category: "main"
  },
  ...
  {
    key: "langchain-config" as const,
    label: "Config LangChain",
    icon: Link,
    category: "config"
  },
  {
    key: "admin-dashboard" as const,
    label: "Admin Dashboard",
    icon: Shield,
    category: "admin"
  },
  {
    key: "admin-profile" as const,
    label: "Perfil Admin",
    icon: Settings,
    category: "admin"
  },
  ...
  {
    key: "admin-customer-edit" as const,
...
export default function NavigationSidebar() {
  ...
  const renderNavItem = (item: typeof navigationItems[0]) => {
    ...
    // Mapear notificações específicas para alguns itens
    let notificationCount = 0
    if (item.key === 'chat' && notifications) {
      notificationCount = notifications.unreadChats
    } else if (item.key === 'bulk-sender' && notifications) {
      notificationCount = notifications.runningCampaigns
    } else if (item.key === 'warmup' && notifications) {
      notificationCount = notifications.pendingWarmups
    }
